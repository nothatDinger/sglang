...
metatags:
    description: "Use HiSparse, ScoutAttention, and InfiniGen to reduce decode GPU KV memory with CPU pinned host storage in regular or PD serving."
---
...
> **Prerequisites**: The legacy `--enable-hisparse` path supports
> **DeepSeek Sparse Attention (DSA)** models (for example DeepSeek-V3.2 and
> GLM-5.1) and **DeepSeek V4**. DeepSeek V4 can also enable the same host-backed
> runtime without the legacy flag by explicitly setting
> `dsv4_prefetch_mode` to `scout` or `infinigen`; this works in a regular
> server as well as on a PD decode server.
...
- <code>scout</code> (default): a non-mutating CUDA classifier replaces misses
...
- <code>infinigen</code>: the predicted target top-k runs the full HiSparse swap-in
...
In <code>scout</code> mode, every <code>dsv4_recall_interval</code> global decode
...
| --- | --- | --- |
| <code>dsv4_prefetch_mode</code> | <code>"scout"</code> | <code>"scout"</code> host-miss attention or <code>"infinigen"</code> side-stream recall |
...
Explicitly setting <code>dsv4_prefetch_mode</code> enables the DeepSeek-V4
host-backed sparse runtime; <code>--enable-hisparse</code> is not required.
Without an explicit mode or the legacy flag, DeepSeek V4 retains its regular
pool and attention path. The old values <code>"cpu"</code> and
<code>"h2d"</code> remain aliases for <code>"scout"</code> and
<code>"infinigen"</code>, respectively, and emit a deprecation warning.
...
<code>dsv4_sparse/*</code> and per-layer mean/p50/p95/max logs for index
...
eager decode (<code>--disable-decode-cuda-graph</code>). On SM120, ScoutAttention mode
...
Example ScoutAttention mode:

~~~bash
--disable-decode-cuda-graph \
--hisparse-config='{"top_k":512,"device_buffer_size":4096,"dsv4_prefetch_mode":"scout","dsv4_recall_interval":8,"dsv4_profile":true}'
~~~

Example InfiniGen mode:

~~~bash
--disable-decode-cuda-graph \
--hisparse-config='{"top_k":512,"device_buffer_size":4096,"dsv4_prefetch_mode":"infinigen"}'
~~~
...
Legacy DSA HiSparse remains a PD-decode deployment. DeepSeek-V4
ScoutAttention/InfiniGen can run either in a regular server or on a PD decode
server. If the same explicit mode config is passed to both PD roles, the
prefill server warns and keeps its regular KV pool while the decode server
activates the host-backed sparse runtime.
...
- A PD prefill instance does not create the DeepSeek-V4 host-backed decode pool.
  An explicit DSV4 mode on prefill is ignored with a warning.
- On DeepSeek V4, an explicit `dsv4_prefetch_mode` activates ScoutAttention or
  InfiniGen without `--enable-hisparse`; the legacy flag without a mode still
  defaults to ScoutAttention.
